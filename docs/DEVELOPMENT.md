...
```bash
uv sync --extra local_voice --group dev
...
# The app starts missing localhost llama.cpp servers automatically.
BACKEND_PROVIDER=local \
PIPER_VOICE=./cache/piper-voices/en_US-lessac-medium.onnx \
uv run reachy-mini-conversation-app --debug
```

Expected startup markers are `Using Local Mac via LocalConversationHandler`, `Starting Local chat model server`,
`Local chat model server ready`, `Audio recording started`, and transcript logs for `role=user` / `role=assistant`.
Stop with Ctrl-C and confirm shutdown reaches `Shutdown complete.`
...
| Tool registry and profile loading | `tests/test_tool_registry.py`, `tests/test_profile_paths.py`, `tests/test_config_name_collisions.py` |
...
For an OpenAI-compatible realtime backend, prefer subclassing `BaseRealtimeHandler` from `backends/base_realtime.py` and implementing:
...
For a backend with a different SDK/event shape, implement `ConversationHandler` from `backends/interface.py` directly, using `GeminiLiveHandler` as the closest example.
...
4. Add or update tests under `tests/tools/` or `tests/test_tool_registry.py`.
...
The main environment variables are documented in `README.md`. In code, runtime config lives in `src/reachy_mini_conversation_app/runtime/config.py` and can be refreshed with `refresh_runtime_config_from_env()` after loading an instance `.env`.

Production backend selection is controlled by `BACKEND_PROVIDER`:
...
OpenAI and Gemini adapters remain legacy-only and should be covered only by deprecation/compatibility tests.
