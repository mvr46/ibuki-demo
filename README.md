...
The default setup uses the local backend. For the optimized Mac mini wired setup, install llama.cpp and a Piper voice;
the robot app starts the three local llama.cpp servers automatically when launched from the dashboard:
...
| `LOCAL_MODEL_SERVER_AUTOSTART` | Start missing localhost llama.cpp chat/router/vision servers when the app starts, defaults to `true`. |
| `LOCAL_MODEL_SERVER_START_TIMEOUT_SECONDS` | Max time to wait for app-managed llama.cpp servers to become ready, defaults to `240`. |
| `LOCAL_LLAMA_SERVER_BIN` | llama.cpp server executable, defaults to `llama-server`. |
| `LOCAL_CHAT_SERVER_HF` | Hugging Face GGUF repo/quant passed to `llama-server -hf` for chat, defaults to `ggml-org/gemma-3-1b-it-GGUF:Q4_K_M`. |
...
| `LOCAL_ROUTER_SERVER_HF` | Hugging Face GGUF repo/quant passed to `llama-server -hf` for routing, defaults to `Qwen/Qwen3-0.6B-GGUF:Q8_0`. |
...
| `LOCAL_VISION_SERVER_HF` | Hugging Face GGUF repo/quant passed to `llama-server -hf` for camera questions, defaults to `ggml-org/SmolVLM2-500M-Video-Instruct-GGUF:Q8_0`. |
