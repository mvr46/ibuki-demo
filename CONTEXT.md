...
The default backend is local (`BACKEND_PROVIDER=local`): MLX Whisper for STT, app-managed llama.cpp/OpenAI-compatible servers for chat, Qwen tool routing, and fast vision, plus Piper for TTS. Hugging Face remains the production fallback. OpenAI Realtime and Gemini Live remain legacy adapters with deprecation warnings.
