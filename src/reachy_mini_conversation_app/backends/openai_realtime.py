...
from reachy_mini_conversation_app.backends.base_realtime import BaseRealtimeHandler, to_realtime_tools_config
...
class OpenaiRealtimeHandler(BaseRealtimeHandler):
    ...
    def __init__(
        ...
        super().__init__(deps, instance_path, startup_voice)
        logger.warning("OpenAI Realtime backend is legacy-only in this production build.")

    async def _prepare_startup_credentials(self) -> None:
        """Validate that OpenAI credentials came from environment/config."""
        if not config.OPENAI_API_KEY:
            logger.warning("OPENAI_API_KEY missing. Legacy OpenAI backend will use placeholder credentials.")

    def _persist_credentials_if_needed(self) -> None:
        """Skip legacy OpenAI credential persistence."""
