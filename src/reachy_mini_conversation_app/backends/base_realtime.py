...
class BaseRealtimeHandler(ConversationHandler, ABC):
    ...
    async def apply_personality(self, profile: str | None) -> str:
        """Apply a new profile at runtime if possible.
        ...
        try:
            ...
            from reachy_mini_conversation_app.runtime.config import config as _config
            ...
            try:
                ...
                logger.error("Failed to resolve profile content: %s", e)
            ...
            if self.connection is not None:
                try:
                    ...
                    logger.info("Applied profile via live update: %s", profile or "built-in default")
                except Exception as e:
                ...
                try:
                    await self._restart_session()
                    return "Applied profile and restarted realtime session."
                except Exception as e:
                    logger.warning("Failed to restart session after apply: %s", e)
                    return "Applied profile. Will take effect on next connection."
            else:
                logger.info(
                    "Applied profile recorded: %s (no live connection; will apply on next session)",
                    profile or "built-in default",
                )
                return "Applied profile. Will take effect on next connection."
        except Exception as e:
            logger.error("Error applying profile '%s': %s", profile, e)
            return f"Failed to apply profile: {e}"
                ...
                AdditionalOutputs(
                    {
                        ...
                        # Dashboard metadata.status accepts only "pending" and "done".
                        "metadata": {
    ...
    async def _run_realtime_session(self) -> None:
        ...
        model_name = get_model_name_for_backend(self.BACKEND_PROVIDER)
        if model_name:
    ...
    async def emit(self) -> Tuple[int, NDArray[np.int16]] | AdditionalOutputs | None:
        ...
        # This is polled by the local robot audio stream.
