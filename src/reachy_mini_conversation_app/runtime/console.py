"""Bidirectional local audio stream with the production settings dashboard.

If the selected backend is missing its required local/Hugging Face target, the
Reachy Mini Apps settings server exposes the static dashboard so users can fix
configuration and restart cleanly.

The settings UI is served from this package's ``static/`` folder. It persists
the selected production backend and profile settings into the app instance's
``.env`` file when available.
"""
...
from reachy_mini_conversation_app.backends.local_tts import piper_tts_status
from reachy_mini_conversation_app.audio.startup_config import apply_audio_startup_config
from reachy_mini_conversation_app.runtime.startup_settings import read_startup_settings, write_startup_settings
...
class LocalStream:
        ...
        try:
            ...
            if template_text is None:
                try:
                    ...
                    template_text = None
            if template_text is None:
                packaged = Path(__file__).parents[1] / ".env.example"
                if packaged.exists():
                    try:
    ...
    def _persist_personality(self, profile: Optional[str], voice_override: Optional[str] = None) -> None:
        ...
        try:
            ...
            logger.info("Persisted startup profile settings to %s", Path(self._instance_path))
        except Exception as e:
            logger.warning("Failed to persist startup profile settings: %s", e)

    def _read_persisted_personality(self) -> Optional[str]:
        """Read the saved startup profile from instance-local UI settings."""
    ...
    def _init_settings_ui_if_needed(self) -> None:
        ...
        see a confirmation message even when the backend is already configured.
        """
        ...
        # GET /status -> backend and runtime readiness
        ...
        def _set_backend(payload: BackendPayload) -> JSONResponse:
            ...
            if backend not in {HF_BACKEND, LOCAL_BACKEND}:
    ...
    def launch(self) -> None:
        ...
        If the selected backend is missing its runtime requirement, expose the
        settings UI via the Reachy Mini settings server before starting streams.
        """
        ...
        # If a legacy backend key is still missing, keep the compatibility wait.
        if not self._has_required_key(active_backend):
            ...
            try:
                ...
                logger.info("Interrupted while waiting for legacy API key.")
                return

        # Start media after the backend requirement is available
        ...
        async def runner() -> None:
            # Capture loop for cross-thread profile actions
            loop = asyncio.get_running_loop()
            self._asyncio_loop = loop  # type: ignore[assignment]
            # Mount profile routes now that loop and handler are available
            try:
